...
let tag_bcolor_exist = "#00cc00";
let tag_bcolor_not_exist = "#c0c0c0";

GM_setValue("tag_bcolor_exist", tag_bcolor_exist);
GM_setValue("tag_bcolor_not_exist", tag_bcolor_not_exist);
GM_setValue("tag_bcolor_need_login", "#ff6666");
GM_setValue("tag_bcolor_error", "#ff6666");
GM_setValue("tag_fcolor_exist", "#ffffff");
GM_setValue("tag_fcolor_not_exist", "#000000");
GM_setValue("tag_fcolor_need_login", "#ffffff");
GM_setValue("tag_fcolor_error", "#ffffff");
...
if (typeof GM_registerMenuCommand !== "undefined") {
    ...
    function changeTagBColor() {
        let now_bcolor_list = [GM_getValue("tag_bcolor_exist", tag_bcolor_exist), GM_getValue("tag_bcolor_not_exist", tag_bcolor_not_exist), GM_getValue("tag_bcolor_need_login", ""), GM_getValue("tag_bcolor_error", "")];
        let name = prompt("请依次输入代表'资源存在','资源不存在','站点需要登陆','站点解析错误'颜色的Hex值，并用英文逗号分割。当前值为：", `${now_bcolor_list.join(',')}`);
        if (name != null && name !== "") {
            try {
                let bcolor_list = name.split(",");
                GM_setValue("tag_bcolor_exist", bcolor_list[0] || tag_bcolor_exist);
                GM_setValue("tag_bcolor_not_exist", bcolor_list[1] || tag_bcolor_not_exist);
